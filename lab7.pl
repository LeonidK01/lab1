...
pr8:-read_str(A,_),prov_w(A,0,Indw),prov_x(A,0,Indx),(Indw<Indx->write("w встречается раньше");write("x встречается раньше")).
...
pr21_1:- write("Введите первую строку."), nl, read_str(List1,_),
	write("Введите вторую строку с разделителями."), nl, read_str(List2,_),
